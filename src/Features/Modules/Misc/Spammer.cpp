...
Spammer::Spammer(): ModuleBase("Spammer", "说藏话", ModuleCategory::Misc, 0, false)
{
    addSetting(&mMode);
    addSetting(&mDelayMs);
...
void Spammer::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    while (StringUtils::contains(entry, "!randMention!"))
    {
        ...
        entry = StringUtils::replace(entry, "!dick!", mentions[mentionIndex]);
        mentionIndex++;
    }

    // replace leftover mentions with empty string
    entry = StringUtils::replaceAll(entry, "!fuck", "");
